...
var GCApp = {
    ...
    /** Make the vertical timeline indicator (in the header) draggable.
     *  Users can grab the handle or the line itself to scrub the current time.
     *  The slider below remains fully functional as a fallback. */
    _setupTimeIndicatorDrag:function(){
        var self=this;
        var head=document.getElementById('gcTimeIndicatorHead');
        if(!head)return;
        ...
        if(tl){
            tl.addEventListener('mousedown',function(e){
                // Only direct clicks on the timeline itself, not on the indicator or milestones
